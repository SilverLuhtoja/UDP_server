...
impl Player {
    pub fn new(cube_size:f32) -> Player {
        Self {x: 0.0, y:0.0, angle: 0.0, speed:0.0, radius: cube_size/4.0, direction_ray_length: cube_size/2.0}    
    }
    pub fn set_position(&mut self, game_window: GameWindow) {
        let position = game_window.get_random_empty_space();
        println!("POSITION {:?}", position);
        self.x = position.0 as f32 * game_window.minimap_cube_size + game_window.minimap_start_x + game_window.minimap_cube_size/2.0;
        self.y = position.1 as f32 * game_window.minimap_cube_size + game_window.minimap_start_y + game_window.minimap_cube_size/2.0;
    }

    pub fn display_room(&self, game_window: GameWindow) {    
        //Draw rays from player
        for (i, ray) in self.get_rays(game_window.clone()).iter().enumerate() {
            //on minimap
            draw_line(self.x, self.y, self.x + ray.angle.cos() * ray.distance, (self.y + ray.angle.sin() * ray.distance) + self.radius, 1.0, YELLOW);

            //visual part
            let distance:f32 = ray.distance;
            let wall_height:f32 = ((game_window.minimap_cube_size * 5.0) / distance) *277.0;
            let mut wall_color:macroquad::color::Color = LIGHTGRAY;
            if ray.vertical {
                wall_color = GRAY;
            }
            //wall
            draw_rectangle(i as f32 + 20.0, game_window.minimap_finish_y/2.0 - wall_height/2.0, 1.0, wall_height, wall_color);
            //floor
            draw_rectangle(i as f32 + 20.0, game_window.minimap_finish_y/2.0 + wall_height/2.0, 1.0, screen_height()/2.0 - wall_height/2.0, YELLOW);
            //ceiling
            draw_rectangle(i as f32 + 20.0, 20.0, 1.0, game_window.minimap_finish_y/2.0 - wall_height/2.0, WHITE);
    ...
    pub fn move_player(&mut self, game_window: GameWindow){
